...
- **Proposal search reads the whole description**: the "Search proposals" box only really looked at proposal titles and the opening words of a description, so a term further down — a prerequisite, a tool named in the third paragraph — found nothing. Descriptions are now searched all the way through. Two related fixes come with it: a phrase that happens to be bold or italic is found as it is written on screen rather than with its formatting marks in the way, and results are matched more strictly than before, so a search returns the handful of proposals that genuinely match instead of half the list. A small typo is still forgiven
- **Searching a long proposal list no longer makes the page stutter**: at an event with a couple of hundred proposals, every letter typed into "Search proposals" left the page unresponsive for a moment, and the same pause showed up when sorting, filtering or voting straight from the list. The list now prepares each proposal's description once instead of redoing that work for every proposal on every keystroke, so searching keeps up with typing
...
- **Hosts see how their proposal did**: opening your own proposal during the scheduling phase now shows a vote breakdown — how many of the event's attendees voted and how many didn't, and how the votes split between ❤️ Interested, ⭐ Maybe and 👋🏽 Skip, each with a percentage. It closes with how many people to expect if you host the session — a range rather than a single number, and even that is a very rough guess. It comes from a formula worked out at a single event, where thirteen hosts were asked afterwards roughly how many people had turned up and their recollections were compared against the votes; it takes a proposal's ❤️ count relative to the other proposals at the event and scales it by the number of attendees. That is hardly any data to build on, so the figure can be well out in either direction — use it to pick a room, not to plan handouts. Expect it to change: the formula will be reworked as more events are recorded, and the assumptions behind it (today a fixed guess at how many sessions run at the same time) are meant to become per-event settings. Nobody should stop voting ⭐ Maybe on account of this — today's formula happens not to use ⭐ votes, but thirteen sessions prove very little either way. No figure is shown when fewer than one attendee in ten voted on a proposal, when nobody voted ❤️, or before the event has its attendees set up. Only the hosts of a proposal see this; a proposal nobody has taken on shows it to everyone, since anyone may still pick it up. Who voted which way is still never shown
- **Dark mode**: the site now has a proper dark theme, chosen with the small System / Light / Dark switch at the bottom of every page (and repeated under "Appearance" on the settings page). "System" — the default — follows whatever your phone or laptop is set to, so the schedule turns dark in the evening on its own. The choice is remembered per device, which means you can read in the dark on your phone and stay light on your laptop, and it applies before you have picked your name, on the login screen and on a kiosk display too. Colours were checked against the WCAG contrast guidelines in both themes, so nothing is a shade of grey on grey
...
- **Show only the attendees who have filled something in**: a new "Has profile" filter next to "Session host" hides the names that carry nothing but a name, which at most events is a good part of the list. The two filters combine, so "hosts who have written something about themselves" is two clicks. Anything an attendee entered themselves counts — a bio, pronouns, where they're based, languages, a photo, an answered conversation starter or a contact detail
...
- **Profiles show the photo big**: a profile led with a small round thumbnail, so the one thing that actually helps you recognise someone was the smallest thing on the page and only got bigger if you thought to click it. The photo is now shown at a proper size, with the name, pronouns, where someone is based and the languages they speak under it, and the bio, prompts and contact details in a column beside it on a wide screen. Scrolling through a long profile keeps the photo and name in view instead of leaving them behind. On a phone, and on a narrow window, everything stacks as before. Clicking the photo still opens it larger. Photos keep the square shape they are stored in: whatever you upload is cropped to a square around its centre, so it is worth uploading a picture with your face roughly in the middle
- **The attendee directory shows who people are, not just their names**: getting a sense of who is coming meant opening every profile in turn and going back to the list each time. Each attendee now brings the first couple of lines of their bio along — or, if they haven't written one, the first conversation starter they answered — so the list can be read straight through. Faces are bigger too, and the whole directory is on one page: no more paging through 25 names at a time. The search box, the filters and the sort choice work exactly as before
- **Read through the attendees one after another**: seeing who is coming meant opening a profile, reading it, going back to the list, finding your place again and opening the next one. A profile now opens over the list instead of replacing it, with **Prev** and **Next** at the top — so you can go straight on to the next person, and closing puts you back exactly where you were, with your search and filters intact. The arrow keys do the same, and Escape closes; on a phone, swiping sideways moves on to the next or the previous person, and stops with a nudge at either end of the list. Prev and Next follow whatever the list is showing, so searching for "German" and then reading through gives you only those attendees; the count at the top ("12 of 87 attendees") says which collection you are in and where. Profiles opened from a session, a proposal or a comment work the same way, over the full list of attendees. Every profile you open is a step in your browser history, so Back retraces the ones you read. The photo now grows and shrinks in place instead of opening a window of its own
- **Searching and filtering the attendee directory is instant**: typing a search, flipping a filter or changing the sort used to reload the whole list from the server and drop you back at the top of it. It now happens on the spot, keeping your place. The address bar still carries what you are looking at, so a filtered or searched list can be bookmarked and shared as before
...
- **"Back to attendees" is now a small link, not a big red button**: on the edit-profile and settings pages, the way back to the attendee list was the loudest thing on the screen, above the heading, as if leaving were the main thing to do there. It is now a quiet "← Attendees" link in the corner, the same as everywhere else in the site
...
- A fitted model for predicting session attendance from voting results, in `docs/dev/attendance-model/`, based on 13 sessions from a 259-attendee event. It predicts a range rather than a number, and the write-up is anonymised so it can be shared with hosts. The headline caveat is that with 13 sessions the vote–attendance correlation is not statistically significant, so it is a best guess rather than an established relationship — counting attendance at the next event is what would settle it
...
- The dev seed (`make dev-db-seed`) now defaults to a `large` profile: 400 guests, ~750 proposals and a mostly filled Gamma schedule, generated deterministically from word lists on top of the unchanged hand-curated fixtures — so dev pages show realistic volumes and slow list rendering surfaces before a real event hits it. The E2E suite always seeds the curated-only `small` profile (`SEED_PROFILE` selects one explicitly), so tests stay fast and can never come to depend on bulk data. The seed script is split into `scripts/seed/` modules: curated data under `data/`, the bulk generator in `bulk.ts`
