...
- **Protect your name**: attendees can secure the name they act as with a password, so on a shared device others can't pick it and act as them. A protected name shows a small lock in the name selector; choosing it asks for the password or a one-time code emailed to the attendee. Turning protection off or changing the password accepts either the password or an emailed code, so a broken mailer can never lock anyone out; whenever protection is turned off or the password changes, a heads-up email goes to the attendee so an unexpected change doesn't go unnoticed. After logging in through the emailed link, an attendee with no password yet is offered to set one right there, reusing the code they just used — no second trip to Settings. Requires SMTP for the emailed-code option
